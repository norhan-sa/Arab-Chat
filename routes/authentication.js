 ...
 router.post('/reg', async(req, res)=>{
  ...
  try{
   ...
   if(!site_data.site_data[0].allow_reg)
   return res.status(400).send({msg: 'غير مسموح تسجيل أعضاء جدد', data: null , status:'400'});

   if( name.length !== site_data.site_data[0].reg_name_letters )
   return res.status(400).send({msg: `عدد الحروف المسموح بها ${site_data.site_data[0].reg_name_letters}`, data: null , status:'400'});
 ...
 router.post('/login', async(req,res)=>{
  ...
  router.post('/visitor', async(req,res)=>{
    ...
    if(!site_data.site_data[0].allow_visitors)
    return res.status(400).send({msg: 'غير مسموح دخول الزوار', data: null , status:'400'});

    if( name.length !== site_data.site_data[0].visitor_name_letters )
    return res.status(400).send({msg: `عدد الحروف المسموح بها ${site_data.site_data[0].visitor_name_letters}`, data: null , status:'400'});
